...
func (s *Server) Serve(listener net.Listener) error {
	if logger != nil {
		logger.Info("Server.Serve starting", zap.String("addr", listener.Addr().String()))
	}
	...
	for {
		...
		if logger != nil {
			logger.Debug("Accepted connection", zap.String("remote", conn.RemoteAddr().String()))
		}
...
func (s *Server) negotiate(conn net.Conn) (bool, error) {
	...
	supported := slices.Contains(req.Methods, serverMethod)
